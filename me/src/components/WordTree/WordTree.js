  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [keywordsResponse, colorsResponse] = await Promise.all([
          axios.get(`${process.env.REACT_APP_API_URL}/users/me/keywords`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          }),
          axios.get(`${process.env.REACT_APP_API_URL}/users/me/colors`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          })
  ...
  const captureScreenshot = async () => {
    if (captureRef.current) {
      try {
        ...
        const token = localStorage.getItem('token');
        await axios.post(`${process.env.REACT_APP_API_URL}/users/tree`, {
